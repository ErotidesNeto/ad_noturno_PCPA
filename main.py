...
def main():
    ...
    st.subheader('Adicional Norturno de Policial Civil')
    ...
    if st.button('Gerar Minuta'):
        if st.session_state.peticao_inicial_path:
            ...
            with st.spinner('Gerando minuta...'):                
                st.session_state.minuta_file_path = criar_novo_documento(chat(st.session_state.peticao_inicial_path))
                end_time = time.time()
                if st.session_state.minuta_file_path:
                    ...
                    st.error('A petição apresentada não é de um caso de demanda de adicional noturno de policial civil') 
        else:
